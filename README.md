# 📊 Project Management Portfolio  
**Fernanda Menezes**  
Project Manager | Agile & Hybrid Projects | Data-Driven

---

## 👩‍💼 Sobre

Engenheira e Doutora em Engenharia com MBA em Gerenciamento de Projetos.  
Atuo há mais de 3 anos com projetos de tecnologia de software, com foco em previsibilidade, controle de cronograma, gestão de riscos e reporte executivo.

Este repositório apresenta exemplos fictícios de governança e acompanhamento estratégico em projetos ágeis.

---

#  Executive Sprint Status Report  
*(Exemplo Fictício – Projeto Ágil)*

## 📌 Informações Gerais

- **Projeto:** Plataforma de Monitoramento de SLA  
- **Cliente / Área:** Operações Digitais  
- **Gerente do Projeto:** Fernanda Menezes  
- **Sprint Atual:** Sprint 05  
- **Período:** 03/02 – 14/02  

---

## 🟢 Status Geral da Sprint

**Situação:** 🟢 On Track  

**Resumo Executivo:**  
A Sprint 05 evolui conforme planejado, com 82% das entregas concluídas até o momento. A velocidade realizada está aderente à média histórica do time, sem impacto no roadmap macro. Os riscos identificados permanecem sob controle e não afetam a previsão de go-live.

---

## 📈 Indicadores-Chave da Sprint

- Velocidade planejada: 40 pontos  
- Velocidade realizada: 33 pontos  
- Aderência à sprint: 95%  
- Impedimentos críticos: 0  
- Lead time médio: 3,2 dias  
- SLA das entregas: 97%  

---

## Status do Roadmap

- Fase atual: Desenvolvimento  
- % macro do projeto concluído: 68%  
- Previsão de Go-live: 28/03  
- Impacto no cronograma geral: Nenhum  

---

## Riscos Estratégicos

1. Dependência de validação do cliente até 18/02  
   - Mitigação: alinhamento agendado e envio prévio de protótipo funcional  

2. Instabilidade potencial na API externa  
   - Mitigação: testes antecipados e plano de contingência definido  

---

## ⚠ Pontos de Decisão

- Priorização de backlog incremental solicitado pelo cliente  
- Aprovação de escopo adicional referente ao módulo de exportação de dados  

---

# Matriz de Risco
*(Exemplo Fictício – Gestão Preventiva)*

## Critério de Classificação

- Probabilidade: Baixa (1) | Média (2) | Alta (3)  
- Impacto: Baixo (1) | Médio (2) | Alto (3)  
- Severidade: Probabilidade x Impacto  

---

## Registro de Riscos

| ID  | Descrição do Risco | Prob. | Impacto | Severidade | Estratégia | Responsável | Status |
|-----|--------------------|-------|----------|------------|-------------|--------------|---------|
| R01 | Atraso na validação do cliente | 2 | 3 | Alto | Mitigar | GP | Monitoramento |
| R02 | Instabilidade na API externa | 2 | 3 | Alto | Mitigar | Tech Lead | Em andamento |
| R03 | Redução de capacidade do time | 1 | 3 | Médio | Mitigar | GP | Sob controle |
| R04 | Mudança de escopo não prevista | 2 | 2 | Médio | Mitigar | GP | Monitorado |
| R05 | Dependência de acesso ao ambiente produtivo | 1 | 2 | Baixo | Aceitar | GP | Resolvido |

---

## Análise Consolidada

- Total de riscos identificados: 5  
- Riscos de alta severidade: 2  
- Riscos mitigados: 1  
- Impacto atual no cronograma: Nenhum  

---

# 🎯 Competências Demonstradas

✔ Gestão híbrida (Agile + PMP)  
✔ Controle executivo de sprint  
✔ Monitoramento de roadmap  
✔ Gestão estruturada de riscos  
✔ Comunicação estratégica para diretoria  
✔ Mentalidade orientada a indicadores  

---

